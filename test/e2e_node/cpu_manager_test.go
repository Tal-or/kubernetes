...
var _ = SIGDescribe("CPU Manager", ginkgo.Ordered, ginkgo.ContinueOnFailure, framework.WithSerial(), feature.CPUManager, func() {
	...
	ginkgo.When("running with SMT Alignment", ginkgo.Label("smt-alignment"), func() {
		...
		ginkgo.It("should count for pre-allocated CPUs after kubelet restart", func(ctx context.Context) {
			...
			if cpuCount < smtLevel {
				e2eskipper.Skipf("Not enough allocatable CPUs for this test: need at least %d, have %d", smtLevel, allocatableCPUs)
			}
			...
			ginkgo.By(fmt.Sprintf("creating the testing pod cpuRequest=%v (allocatable=%d, reserved=%d)", cpuRequest, allocatableCPUs, reservedCPUs.Size()))
			pod := makeCPUManagerPod("gu-pod", []ctnAttribute{
				{
					ctnName:    "gu-container",
					cpuRequest: cpuRequest,
			...
			framework.Logf("CPUs after kubelet restart: %s (count=%d)", cpusAfterRestart.String(), cpusAfterRestart.Size())
			gomega.Expect(cpusAfterRestart).To(gomega.Equal(cpusBeforeRestart), "CPUs should be the same after kubelet restart")

			ginkgo.By("verifying the pod was not rejected with SMTAlignmentError")
			gomega.Expect(pod).ToNot(HaveStatusReasonMatchingRegex(`SMT.*Alignment.*Error`))
		})
	})
